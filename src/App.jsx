  ...
  return (
    <>
      <section className="lg:flex items-center justify-items-center h-screen">
        ...
        {/* UI for Notes */}
        <div className="lg:w-1/2 h-screen lg:border-l-4 border-dashed border-black max-lg:border-t-4 bg-gray-300 flex flex-wrap items-center p-5 gap-6 overflow-auto">

          {Tasks.map((task, id) => {
            return (
              <div className="w-full max-w-72 h-72 bg-white rounded-4xl shadow-2xl flex items-end justify-center p-2 relative m-3" key={id} >
                <img
                  src="./pin.png"
                  alt="pin image"
                ...
                <h1 className="text-5xl font-semibold text-center  my-1">{task.Heading}</h1>
                <p className="text-gray-400 font-medium text-xl m-2">{task.List}</p>         
                </div>
              </div>
            );
          })}
